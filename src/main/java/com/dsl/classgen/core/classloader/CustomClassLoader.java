...
public class CustomClassLoader extends ClassLoader {
	...
	private String pathFormatToBinaryNameFormat(String value) {
		// assumes the path contains the file name and replaces file separators with dots and removes the file extension
		Path filePath = Path.of(value);
		Path fileName = filePath.getFileName();
		Path parentPath = filePath.subpath(2, filePath.getNameCount() - 1);
		
		String binaryName = Utils.normalizePath(parentPath, File.separator, ".").toString().concat("." + fileName.toString());
