...
/**
 * The Class OutterClassGenerator.
 */
public final class OutterClassGenerator extends SupportProvider {
	...
	public void generateData() {
		...
		outterClassUnit.getClassByName(pathsCtx.getOutterClassName())
				.ifPresent(c -> generateInnerClasses().forEach(c::addMember));
		pathsCtx.setGeneratedClass(outterClassUnit.toString());
	}

	/**
	 * Generate outter class.
	 *
	 * @return the compilation unit containing the generated outter class
	 */
	private CompilationUnit generateOutterClass() {
		CompilationUnit cUnit = new CompilationUnit();

		cUnit.setPackageDeclaration(pathsCtx.getPackageClass()).addImport(GeneratedOutterClass.class)
				...
				.addAnnotation(GeneratedOutterClass.class).addConstructor(Keyword.PRIVATE)
	 ...
	 *
	 * @return the list of generated static inner classes
	 */
